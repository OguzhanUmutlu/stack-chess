...
const GetAllMoveList = {
    p(piece, x, y, board, mv) {
        ...
        if (y === (piece.type[0] === "w" ? 6 : 1) && !board.get(x, y + 2 * k)[0] && !board.get(x, y + k)[0]) {
            mv(x, y + k * 2);
        }
        const diaLeft = board.get(x - 1, y + k)[0];
        const diaRight = board.get(x + 1, y + k)[0];
        if (diaLeft  && diaLeft.type[0]  !== piece.type[0]) mv(x - 1, y + k);
        if (diaRight && diaRight.type[0] !== piece.type[0]) mv(x + 1, y + k);
